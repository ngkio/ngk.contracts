#include <ngk.system/ngk.system.hpp>
#include <ngk.token/ngk.token.hpp>

#include <ngk/crypto.hpp>
#include <ngk/dispatcher.hpp>

#include <cmath>

namespace ngksystem {

   using ngk::current_time_point;
   using ngk::token;
    ...
    _rexorders(get_self(), get_self().value)
   {
      ...
      _gstate2 = _global2.exists() ? _global2.get() : ngk_global_state2{};
      _gstate3 = _global3.exists() ? _global3.get() : ngk_global_state3{};
      _gstate4 = _global4.exists() ? _global4.get() : get_default_inflation_parameters();
   }

   ngk_global_state system_contract::get_default_parameters() {
      ngk_global_state dp;
      get_blockchain_parameters(dp);
      return dp;
   }

   ngk_global_state4 system_contract::get_default_inflation_parameters() {
      ngk_global_state4 gs4;
   ...
   void system_contract::update_ram_supply() {
      auto cbt = ngk::current_block_time();
   ...
   void system_contract::setparams( const ngk::blockchain_parameters& params ) {
      require_auth( get_self() );
      (ngk::blockchain_parameters&)(_gstate) = params;
   ...
   void system_contract::activate( const ngk::checksum256& feature_digest ) {
      require_auth( get_self() );
   ...
   void native::newaccount( const name&       creator,
      ...
      userres.emplace( newact, [&]( auto& res ) {
        res.owner = newact;
        res.ram_bytes  = newaccount_ram;
        res.net_weight = asset( 0, system_contract::get_core_symbol() );
        res.cpu_weight = asset( 0, system_contract::get_core_symbol() );
      });

      if( creator != get_self() ) {
         check(creator == system_contract::new_account, "you have no privilege to create account");

         //init account has 100k ram, 0 Net, 0 CPU
         set_resource_limits(newact, newaccount_ram, 0, 0);

         {
            system_contract::delegatebw_action delegatebw_act{get_self(),
                                                              {{get_self(), system_contract::active_permission}}};
            delegatebw_act.send(get_self(), newact, asset(newaccount_net_stake, system_contract::get_core_symbol()),
                                asset(newaccount_cpu_stake, system_contract::get_core_symbol()), false);
         }
      }
      else
      {
   ...
   void native::setabi( const name& acnt, const std::vector<char>& abi ) {
      ngk::multi_index< "abihash"_n, abi_hash >  table(get_self(), get_self().value);
      auto itr = table.find( acnt.value );
      if( itr == table.end() ) {
         table.emplace( acnt, [&]( auto& row ) {
            row.owner = acnt;
            row.hash = ngk::sha256(const_cast<char*>(abi.data()), abi.size());
         });
      } else {
   ...
   void system_contract::init( unsigned_int version, const symbol& core ) {
      ...
      auto system_token_supply   = ngk::token::get_supply(token_account, core.code() );
