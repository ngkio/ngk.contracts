...
namespace ngksystem {
   ...
   void system_contract::buyrambytes( const name& payer, const name& receiver, uint32_t bytes ) {
      ...
      const int64_t ngk_reserve   = itr->quote.balance.amount;
      const int64_t cost          = exchange_state::get_bancor_input( ram_reserve, ngk_reserve, bytes );
   ...
   void system_contract::buyram( const name& payer, const name& receiver, const asset& quant )
   {
      ...
      if( res_itr ==  userres.end() ) {
         res_itr = userres.emplace( receiver, [&]( auto& res ) {
               ...
               res.ram_bytes = newaccount_ram + bytes_out;
            });
      } else {
  ...
  /**
   ...
   void system_contract::sellram( const name& account, int64_t bytes ) {
      ...
      check( res_itr->ram_bytes - newaccount_ram >= bytes, "insufficient quota" );
                                   ...
                                   const asset& stake_net_delta, const asset& stake_cpu_delta, bool transfer )
   {
      ...
      if ( stake_account != source_stake_from ) { //for ngk both transfer and refund make no sense
         ...
         if ( need_deferred_trx ) {
            ...
            ngk::cancel_deferred( from.value ); // TODO: Remove this line when replacing deferred trxs is fixed
            out.send( from.value, from, true );
         } else {
            ngk::cancel_deferred( from.value );
         }
