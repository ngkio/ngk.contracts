...
namespace ngksystem {
   ...
   using ngk::microseconds;
   using ngk::singleton;

   void system_contract::register_producer( const name& producer, const ngk::block_signing_authority& producer_authority, const std::string& url, uint16_t location ) {
      auto prod = _producers.find( producer.value );
      const auto ct = current_time_point();

      ngk::public_key producer_key{};
   ...
   void system_contract::regproducer( const name& producer, const ngk::public_key& producer_key, const std::string& url, uint16_t location ) {
   ...
   void system_contract::regproducer2( const name& producer, const ngk::block_signing_authority& producer_authority, const std::string& url, uint16_t location ) {
      require_auth( producer );
      check( url.size() < 512, "url too long" );
   ...
   void system_contract::update_elected_producers( const block_timestamp& block_time ) {
      ...
      using value_type = std::pair<ngk::producer_authority, uint16_t>;
      std::vector< value_type > top_producers;
      top_producers.reserve(21);

      for( auto it = idx.cbegin(); it != idx.cend() && top_producers.size() < 21 && 0 < it->total_votes && it->active(); ++it ) {
         top_producers.emplace_back(
            ngk::producer_authority{
      ...
      std::vector<ngk::producer_authority> producers;
