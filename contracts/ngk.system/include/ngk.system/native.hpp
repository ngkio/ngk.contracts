...
#include <ngk/contract.hpp>
#include <ngk/crypto.hpp>
#include <ngk/fixed_bytes.hpp>
...
namespace ngksystem {
   ...
   using ngk::checksum256;
   using ngk::ignore;
   using ngk::name;
   using ngk::permission_level;
   using ngk::public_key;
   ...
   struct permission_level_weight {
      ...
      NGKLIB_SERIALIZE( permission_level_weight, (permission)(weight) )
   };
   ...
   struct key_weight {
      ...
      // explicit serialization macro is not necessary, used here only to improve compilation time
      NGKLIB_SERIALIZE( key_weight, (key)(weight) )
   };
   ...
   struct wait_weight {
      ...
      // explicit serialization macro is not necessary, used here only to improve compilation time
      NGKLIB_SERIALIZE( wait_weight, (wait_sec)(weight) )
   };
   ...
   struct authority {
      ...
      // explicit serialization macro is not necessary, used here only to improve compilation time
      NGKLIB_SERIALIZE( authority, (threshold)(keys)(accounts)(waits) )
   };
   ...
   struct block_header {
      ...
      std::optional<ngk::producer_schedule>   new_producers;

      // explicit serialization macro is not necessary, used here only to improve compilation time
      NGKLIB_SERIALIZE(block_header, (timestamp)(producer)(confirmed)(previous)(transaction_mroot)(action_mroot)
   ...
   /**
    * The NGK core `native` contract that governs authorization and contracts' abi.
    */
   class [[ngk::contract("ngk.system")]] native : public ngk::contract {
      public:
         ...
         /**
          * @{
          * These actions map one-on-one with the ones defined in core layer of NGK, that's where their implementation
          * actually is done.
          ...
          * This is useful because when doing authorization checks, the NGK based blockchain starts with the
          * action needed to be authorized (and the contract belonging to), and looks up which permission
          * is needed to pass authorization validation. If a link is set, that permission is used for authoraization
          * validation otherwise then active is the default, with the exception of `ngk.any`.
          * `ngk.any` is an implicit permission which exists on every account; you can link actions to `ngk.any`
         ...
         using newaccount_action = ngk::action_wrapper<"newaccount"_n, &native::newaccount>;
         using updateauth_action = ngk::action_wrapper<"updateauth"_n, &native::updateauth>;
         using deleteauth_action = ngk::action_wrapper<"deleteauth"_n, &native::deleteauth>;
         using linkauth_action = ngk::action_wrapper<"linkauth"_n, &native::linkauth>;
         using unlinkauth_action = ngk::action_wrapper<"unlinkauth"_n, &native::unlinkauth>;
         using canceldelay_action = ngk::action_wrapper<"canceldelay"_n, &native::canceldelay>;
         using setcode_action = ngk::action_wrapper<"setcode"_n, &native::setcode>;
         using setabi_action = ngk::action_wrapper<"setabi"_n, &native::setabi>;
   };
   /** @}*/ // @addtogroup ngksystem
}
