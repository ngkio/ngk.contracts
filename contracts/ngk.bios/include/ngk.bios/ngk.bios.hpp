 ...
 * NGK Contracts
 *
 * The design of the NGK blockchain calls for a number of smart contracts that are run at a
 ...
 * managing, and/or using an NGK blockchain. They are provided for reference purposes:
 * - ngk.bios
 * - ngk.system
 * - ngk.msig
 * - ngk.wrap
...
namespace ngkbios {
   ...
   class [[ngk::contract("ngk.bios")]] bios : public ngk::contract {
      public:
          ...
          */
         [[ngk::action]]
         void setprods( const std::vector<ngk::producer_authority>& schedule );
          ...
          */
         [[ngk::action]]
         void reqactivated( const ngk::checksum256& feature_digest );

         struct [[ngk::table]] abi_hash {
            name              owner;
            checksum256       hash;
         ...
         typedef ngk::multi_index< "abihash"_n, abi_hash > abi_hash_table;
